...
export default function PublicProfile({ userId, isPublic = false, onEdit }) {
  ...
  if (loading) return (
    <div style={{ minHeight: "60vh", display: "flex", alignItems: "center", justifyContent: "center" }}>
      <p style={{ color: COLORS.textMuted, fontFamily: FONT, fontSize: 16 }}>Profil wird geladen…</p>
    </div>
  );

  if (!profile) return (
    <div style={{ minHeight: "60vh", display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", gap: 16 }}>
      <p style={{ color: COLORS.textMuted, fontFamily: FONT, fontSize: 16 }}>Profil nicht gefunden.</p>
      ...
      {!isPublic && (
        <div style={{ display: "flex", justifyContent: "flex-end", gap: 10, marginBottom: 20 }} className="no-print">
          <button onClick={() => window.print()} style={{
            ...
            color: COLORS.textSecondary, padding: "10px 18px", borderRadius: 10,
        ...
        <div style={{ display: "flex", gap: 24, alignItems: "flex-start" }} className="hero-flex">
          ...
          <div style={{ flex: 1 }}>
            ...
            <div style={{ display: "flex", flexWrap: "wrap", gap: 8 }}>
              ...
              {profile.work_model && (
                <span style={{ background: COLORS.bgInput, color: COLORS.textSecondary,
      ...
      {(profile.experience || profile.education || profile.languages) && (
        <div style={card}>
          ...
          <Section title="Berufserfahrung" count={profile.experience ? 1 : 0} accent={COLORS.yellow}>
            <p style={{ fontSize: 15, color: COLORS.textSecondary, lineHeight: 1.8, whiteSpace: "pre-wrap" }}>
              {profile.experience}
            </p>
          </Section>
          ...
          <Section title="Sprachen" count={profile.languages ? 1 : 0} accent={COLORS.yellow}>
            <p style={{ fontSize: 15, color: COLORS.textSecondary, lineHeight: 1.8 }}>
              {profile.languages}
            </p>
          </Section>
        </div>
