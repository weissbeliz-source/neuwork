...
function PublicProfile({ userId }) {
  ...
  if (notFound) return (
    <div style={{ minHeight: "100vh", background: "#F5F0E8", display: "flex", alignItems: "center", justifyContent: "center" }}>
      <div style={{ textAlign: "center" }}>
        <p style={{ fontFamily: "'Playfair Display', serif", fontSize: 24, color: "#888" }}>Profil nicht gefunden.</p>
        <a href="/" style={{ fontFamily: "Source Sans 3", color: "#8B7EC8", fontSize: 14, marginTop: 12, display: "block" }}>← Zurück zu mole</a>
      </div>
    </div>
  );
  ...
  if (!pub) return (
    <div style={{ minHeight: "100vh", background: "#F5F0E8", display: "flex", alignItems: "center", justifyContent: "center" }}>
      <span style={{ fontFamily: "'Playfair Display', serif", fontSize: 24, color: "#888" }}>Laden...</span>
    </div>
  );

  const initial = (pub.full_name?.[0] || "M").toUpperCase();

  return (
    <div style={{ fontFamily: "'Georgia', serif", background: "#F5F0E8", minHeight: "100vh", color: "#2C2C2C" }}>
      <style>{`@import url('https://fonts.googleapis.com/css2?family=Playfair+Display:wght@400;600;700&family=Source+Sans+3:wght@300;400;500;600&display=swap'); * { box-sizing: border-box; margin: 0; padding: 0; } .section-label { font-family: 'Source Sans 3', sans-serif; font-size: 11px; font-weight: 600; text-transform: uppercase; letter-spacing: 1.5px; color: #888; margin-bottom: 10px; } .pub-tag { display: inline-block; padding: 5px 12px; border-radius: 20px; font-size: 13px; font-family: 'Source Sans 3', sans-serif; background: #EEE8DC; color: #2C2C2C; margin: 3px; }`}</style>

      {/* Header */}
      <header style={{ background: "#F5F0E8", borderBottom: "1px solid #E2DBD0", padding: "0 24px", height: 64, display: "flex", alignItems: "center", justifyContent: "space-between", maxWidth: 800, margin: "0 auto" }}>
        ...
        <a href="/" style={{ fontFamily: "Source Sans 3", fontSize: 13, color: "#888", textDecoration: "none" }}>← Zur App</a>
      </header>
      ...
      <main style={{ maxWidth: 800, margin: "0 auto", padding: "40px 24px" }}>
        ...
        <div style={{ background: "white", borderRadius: 20, padding: 32, marginBottom: 20, boxShadow: "0 2px 8px rgba(0,0,0,0.06)" }}>
          <div style={{ display: "flex", gap: 20, alignItems: "center", marginBottom: 20 }}>
            {pub.avatar_url ? (
              <img src={pub.avatar_url} alt="avatar" style={{ width: 80, height: 80, borderRadius: "50%", objectFit: "cover" }} />
            ) : (
              <div style={{ width: 80, height: 80, borderRadius: "50%", background: "#8B7EC8", display: "flex", alignItems: "center", justifyContent: "center", flexShrink: 0 }}>
                <span style={{ color: "white", fontSize: 28, fontWeight: 600, fontFamily: "Source Sans 3" }}>{initial}</span>
              </div>
            )}
            <div>
              <h1 style={{ fontFamily: "'Playfair Display', serif", fontSize: 28, fontWeight: 700, marginBottom: 4 }}>{pub.full_name || "Anonym"}</h1>
              <p style={{ fontFamily: "Source Sans 3", fontSize: 15, color: "#666" }}>{pub.headline}</p>
              {pub.looking_for_work && (
                <span style={{ display: "inline-block", marginTop: 8, background: "#EEF7F1", color: "#2D7A4F", padding: "4px 12px", borderRadius: 12, fontSize: 12, fontFamily: "Source Sans 3", fontWeight: 600 }}>✓ Sucht aktuell aktiv</span>
              )}
            </div>
          </div>
          {pub.work_model && (
            <div style={{ fontFamily: "Source Sans 3", fontSize: 14, color: "#888" }}>📍 {pub.work_model}</div>
          )}
        </div>

        {/* Stärken */}
        {(pub.strengths?.tags?.length > 0 || pub.strengths?.custom) && (
          <div style={{ background: "white", borderRadius: 20, padding: 28, marginBottom: 16, boxShadow: "0 2px 8px rgba(0,0,0,0.06)" }}>
            <p className="section-label">Stärken</p>
            <div>{pub.strengths.tags.map(t => <span key={t} className="pub-tag">{t}</span>)}</div>
            {pub.strengths.custom && <p style={{ fontFamily: "Source Sans 3", fontSize: 14, color: "#555", marginTop: 8 }}>{pub.strengths.custom}</p>}
          </div>
        )}

        {/* Arbeitsweise */}
        {(pub.work_style?.tags?.length > 0 || pub.work_style?.custom) && (
          <div style={{ background: "white", borderRadius: 20, padding: 28, marginBottom: 16, boxShadow: "0 2px 8px rgba(0,0,0,0.06)" }}>
            <p className="section-label">Wie ich arbeite</p>
            <div>{pub.work_style.tags.map(t => <span key={t} className="pub-tag">{t}</span>)}</div>
            {pub.work_style.custom && <p style={{ fontFamily: "Source Sans 3", fontSize: 14, color: "#555", marginTop: 8 }}>{pub.work_style.custom}</p>}
          </div>
        )}

        {/* Bedürfnisse */}
        {(pub.needs?.tags?.length > 0 || pub.needs?.custom) && (
          <div style={{ background: "white", borderRadius: 20, padding: 28, marginBottom: 16, boxShadow: "0 2px 8px rgba(0,0,0,0.06)" }}>
            <p className="section-label">Was ich brauche</p>
            <div>{pub.needs.tags.map(t => <span key={t} className="pub-tag">{t}</span>)}</div>
            {pub.needs.custom && <p style={{ fontFamily: "Source Sans 3", fontSize: 14, color: "#555", marginTop: 8 }}>{pub.needs.custom}</p>}
          </div>
        )}

        {/* Skills */}
        {(pub.skills?.tags?.length > 0 || pub.skills?.custom) && (
          <div style={{ background: "white", borderRadius: 20, padding: 28, marginBottom: 16, boxShadow: "0 2px 8px rgba(0,0,0,0.06)" }}>
            <p className="section-label">Skills</p>
            <div>{pub.skills.tags.map(t => <span key={t} style={{ display: "inline-block", padding: "5px 14px", borderRadius: 20, background: "#2C2C2C", color: "#F5F0E8", fontFamily: "Source Sans 3", fontSize: 13, margin: 3 }}>{t}</span>)}</div>
            {pub.skills.custom && <p style={{ fontFamily: "Source Sans 3", fontSize: 14, color: "#555", marginTop: 8 }}>{pub.skills.custom}</p>}
          </div>
        )}

        <div style={{ textAlign: "center", marginTop: 32 }}>
          <p style={{ fontFamily: "Source Sans 3", fontSize: 13, color: "#aaa", marginBottom: 8 }}>Dieses Profil wurde mit mole erstellt</p>
          <a href="/" style={{ fontFamily: "Source Sans 3", fontSize: 14, color: "#8B7EC8", textDecoration: "none", fontWeight: 500 }}>Auch bei mole registrieren →</a>
        </div>
      </main>
    </div>
  );
...
export default function App() {
  ...
  // Öffentliche Profilseite erkennen
  ...
  function copyProfileLink() {
    const { data: { user: currentUser } } = supabase.auth.getUser().then(({ data }) => {
      const url = `${window.location.origin}/profil/${data.user.id}`;
