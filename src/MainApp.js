  ...
  return (
    <div style={{ marginBottom: 4 }}>
      <button type="button" onClick={() => setOpen(o => !o)} aria-expanded={open} style={{
        ...
        <span style={{ fontWeight: 600, fontSize: 13, textTransform: "uppercase", letterSpacing: "0.08em", color: COLORS.textMuted }}>{label}</span>
        ...
        <div style={{ background: "#0B1120", border: `1.5px solid ${COLORS.purple}`, borderTop: "none", borderRadius: "0 0 10px 10px", padding: "16px 18px" }}>
          {categories ? (
            <div style={{ display: "grid", gap: 16 }}>
              {categories.map(cat => (
                <div key={cat.title}>
                  <p style={{ fontSize: 11, fontWeight: 700, textTransform: "uppercase", letterSpacing: "0.08em", color: COLORS.textMuted, marginBottom: 8 }}>{cat.title}</p>
...
export default function MainApp() {
  ...
  // Styles
  const inp = (extra = {}) => ({ width: "100%", padding: "13px 16px", background: COLORS.bgInput, border: `1.5px solid ${COLORS.border}`, borderRadius: 10, color: COLORS.textPrimary, fontFamily: FONT, fontSize: 15, ...extra });
  const lbl = { display: "block", fontSize: 12, fontWeight: 600, textTransform: "uppercase", letterSpacing: "0.08em", color: COLORS.textMuted, marginBottom: 7 };
  const sectionHead = (color = COLORS.purple) => ({ fontSize: 15, fontWeight: 700, color, marginBottom: 16, marginTop: 4, paddingBottom: 8, borderBottom: `1px solid ${COLORS.border}` });

  if (loading) return (
    <div style={{ minHeight: "100vh", background: COLORS.bg, display: "flex", alignItems: "center", justifyContent: "center" }}>
      <p style={{ color: COLORS.textMuted, fontFamily: FONT, fontSize: 18 }}>Lädt…</p>
    </div>
  );
  ...
  return (
    <div style={{ minHeight: "100vh", background: COLORS.bg, color: COLORS.textPrimary, fontFamily: FONT }}>
        ...
        <div style={{ maxWidth: 1100, margin: "0 auto", padding: "0 24px", height: 64, display: "flex", alignItems: "center", justifyContent: "space-between", gap: 16 }}>
          ...
          <div style={{ display: "flex", alignItems: "center", gap: 12, flexShrink: 0 }}>
            ...
            <div>
              ...
              <p style={{ fontSize: 10, color: COLORS.textMuted, letterSpacing: "0.04em" }}>Different.Inclusion</p>
            </div>
          </div>
          ...
          <nav style={{ display: "flex", gap: 2 }}>
            {NAV_ITEMS.map(item => (
              <button key={item.id} className="nav-btn" onClick={() => { setActiveNav(item.id); if (item.id === "profil") setProfileMode("view"); }}
                style={{ padding: "8px 16px", borderRadius: 8, border: "none", cursor: "pointer", fontSize: 14, fontWeight: 500, background: activeNav === item.id ? COLORS.purpleBg : "transparent", color: activeNav === item.id ? COLORS.purple : COLORS.textMuted, borderBottom: `2px solid ${activeNav === item.id ? COLORS.purple : "transparent"}`, transition: "all 0.15s" }}>
          ...
          <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
            ...
            <button onClick={handleLogout} style={{ background: "none", border: `1px solid ${COLORS.border}`, borderRadius: 8, padding: "7px 14px", cursor: "pointer", fontSize: 13, color: COLORS.textMuted }}>
              Logout
            </button>
          </div>
        </div>
      ...
      <main style={{ maxWidth: activeNav === "profil" ? 860 : 1100, margin: "0 auto", padding: "36px 24px 80px" }}>
        ...
        {activeNav === "profil" && (
          <div>
            ...
            <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", marginBottom: 28, flexWrap: "wrap", gap: 12 }}>
              <div>
                <h1 style={{ fontSize: 28, fontWeight: 700, letterSpacing: "-0.5px", marginBottom: 4 }}>Mein Profil</h1>
              ...
              <div style={{ display: "flex", gap: 10, flexWrap: "wrap" }}>
                {profileMode === "view" && (
                  <>
                    {/* Link teilen */}
                    <button onClick={copyProfileLink} style={{ background: "transparent", border: `1.5px solid ${COLORS.border}`, color: COLORS.textSecondary, padding: "10px 18px", borderRadius: 10, cursor: "pointer", fontSize: 14, fontWeight: 500 }}>
                ...
                {profileMode === "edit" && (
                  <>
                    <button onClick={() => { setProfileMode("view"); setSaveMessage(""); }} style={{ background: "transparent", border: `1.5px solid ${COLORS.border}`, color: COLORS.textMuted, padding: "10px 18px", borderRadius: 10, cursor: "pointer", fontSize: 14 }}>
                      Abbrechen
                    </button>
            ...
            {/* BEARBEITUNGSMODUS */}
            {profileMode === "edit" && (
              <div style={{ background: COLORS.bgCard, border: `2px solid ${COLORS.purple}`, borderRadius: 16, padding: "32px 36px" }}>
                {profileLoading && <p style={{ color: COLORS.textMuted, marginBottom: 16, fontSize: 14 }}>Profil wird geladen…</p>}
                ...
                <div style={{ display: "grid", gap: 24 }}>
                  ...
                  <div style={{ borderTop: `1px solid ${COLORS.border}`, paddingTop: 24 }}>
                    <p style={sectionHead(COLORS.purple)}>Wer ich bin</p>
                    <div style={{ display: "grid", gap: 16 }}>
                      ...
                      <div><label style={lbl}>Über mich</label><textarea value={profile.bio} onChange={e => setProfile({ ...profile, bio: e.target.value })} placeholder="Was treibt mich an? Was macht mich besonders?" rows={3} style={inp({ resize: "vertical" })} /></div>
                    </div>
                  </div>
                  ...
                  <div style={{ borderTop: `1px solid ${COLORS.border}`, paddingTop: 24 }}>
                    ...
                    <div style={{ display: "grid", gap: 8 }}>
                      ...
                      <TagField label="Besondere Fähigkeiten" options={TAG_OPTIONS.strengths_special} value={profile.strengths_special} onChange={v => setProfile({ ...profile, strengths_special: v })} placeholder="z.B. Hyperfokus, Mustererkennung…" />
                      <div style={{ marginTop: 4 }}>
                        <label style={lbl}>Spezialinteressen</label>
                        <textarea value={profile.special_interests} onChange={e => setProfile({ ...profile, special_interests: e.target.value })} placeholder="z.B. Systemdenken, Klimaschutz, Astronomie…" rows={2} style={inp({ resize: "vertical" })} />
                      </div>
                    </div>
                  </div>
                  ...
                  {/* Arbeitsweise */}
                  <div style={{ borderTop: `1px solid ${COLORS.border}`, paddingTop: 24 }}>
                    <p style={sectionHead(COLORS.yellow)}>Wie ich arbeite</p>
                    <div style={{ display: "grid", gap: 8 }}>
                  ...
                  {/* Bedürfnisse */}
                  <div style={{ borderTop: `1px solid ${COLORS.border}`, paddingTop: 24 }}>
                    <p style={sectionHead(COLORS.purple)}>Was ich brauche</p>
                  ...
                  {/* Werdegang */}
                  <div style={{ borderTop: `1px solid ${COLORS.border}`, paddingTop: 24 }}>
                    <p style={sectionHead(COLORS.yellow)}>Werdegang</p>
                    <div style={{ display: "grid", gap: 16 }}>
                      <div>
                        <label style={lbl}>Berufserfahrung</label>
                        <p style={{ fontSize: 12, color: COLORS.textMuted, marginBottom: 6 }}>Format: "MM.JJJJ – MM.JJJJ Firma\nPosition\n- Aufgabe"</p>
                        <textarea value={profile.experience} onChange={e => setProfile({ ...profile, experience: e.target.value })} placeholder={"09.2022 – heute Beispiel GmbH\nSachbearbeitung\n- Aufgabe 1\n- Aufgabe 2"} rows={6} style={inp({ resize: "vertical" })} />
                      </div>
                      <div>
                        <label style={lbl}>Bildungsweg (Ausbildung, Studium, Kurse…)</label>
                        <textarea value={profile.education} onChange={e => setProfile({ ...profile, education: e.target.value })} placeholder={"2018 – 2022 Hochschule XY\nStudiengang\n- Schwerpunkte"} rows={4} style={inp({ resize: "vertical" })} />
                      </div>
                  ...
                  {/* Verfügbarkeit */}
                  <div style={{ borderTop: `1px solid ${COLORS.border}`, paddingTop: 24 }}>
                    <p style={sectionHead(COLORS.green)}>Verfügbarkeit & Kontakt</p>
                    <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 16, marginBottom: 16 }} className="two-col">
                      <div><label style={lbl}>Verfügbar ab</label><input type="text" value={profile.availability} onChange={e => setProfile({ ...profile, availability: e.target.value })} placeholder="z.B. Ab sofort, Ab 01.06.2026" style={inp()} /></div>
                    ...
                    <div style={{ marginBottom: 16 }}>
                      <label style={lbl}>Kontakt (wird nicht öffentlich angezeigt)</label>
                      <input type="text" value={profile.contact_info} onChange={e => setProfile({ ...profile, contact_info: e.target.value })} placeholder="E-Mail, LinkedIn, Portfolio…" style={inp()} />
                    </div>
                    <label style={{ display: "flex", alignItems: "center", gap: 12, fontSize: 15, color: COLORS.textSecondary, cursor: "pointer" }}>
                  ...
                  {/* Als Vorbild */}
                  <div style={{ borderTop: `1px solid ${COLORS.border}`, paddingTop: 24 }}>
                    <p style={sectionHead(COLORS.yellow)}>Als Vorbild sichtbar sein</p>
                    <p style={{ fontSize: 14, color: COLORS.textMuted, marginBottom: 14 }}>Zeige dein Profil in der Vorbilder-Galerie — ohne Kontaktdaten.</p>
                    <label style={{ display: "flex", alignItems: "center", gap: 12, fontSize: 15, color: COLORS.textSecondary, cursor: "pointer", marginBottom: 14 }}>
                    ...
                    {profile.is_rolemodel && (
                      <div>
                        <label style={lbl}>Meine Tags</label>
                          ...
                          {ROLEMODEL_TAGS.map(tag => {
                            ...
                            return (
                              <button key={tag} type="button" onClick={() => {
                                const next = sel ? curr.filter(t => t !== tag) : [...curr, tag];
                                setProfile({ ...profile, rolemodel_tags: next.join(", ") });
                              }} style={{ padding: "6px 14px", borderRadius: 8, border: `1.5px solid ${sel ? COLORS.yellow : COLORS.border}`, background: sel ? COLORS.yellowBg : "transparent", color: sel ? COLORS.yellow : COLORS.textMuted, fontSize: 13, cursor: "pointer" }}>
                                {tag}
                              </button>
                            );
                          })}
                  ...
                  {/* Fördercheck */}
                  <div style={{ borderTop: `1px solid ${COLORS.border}`, paddingTop: 24 }}>
                    <p style={sectionHead()}>Für den Fördercheck (nur für dich)</p>
                    <label style={{ display: "flex", alignItems: "center", gap: 12, fontSize: 15, color: COLORS.textSecondary, cursor: "pointer", marginBottom: 14 }}>
                    ...
                    {profile.has_disability_id && (
                      <div><label style={lbl}>Grad der Behinderung (GdB)</label><input type="text" value={profile.disability_degree} onChange={e => setProfile({ ...profile, disability_degree: e.target.value })} placeholder="z.B. 50" style={inp({ maxWidth: 200 })} /></div>
                    )}
                  </div>

                  {/* Save + Fehler */}
                  {saveMessage && (
                    <div style={{ padding: "12px 18px", borderRadius: 10, background: saveMessage.includes("✓") ? COLORS.greenBg : COLORS.redBg, color: saveMessage.includes("✓") ? COLORS.green : COLORS.red, border: `1px solid ${saveMessage.includes("✓") ? COLORS.greenBorder : COLORS.redBg}`, fontSize: 15 }}>
                      {saveMessage}
                    </div>
                  )}
                  <div style={{ display: "flex", gap: 12, paddingTop: 8, flexWrap: "wrap" }}>
                    ...
                    <button onClick={() => { setProfileMode("view"); setSaveMessage(""); }} style={{ background: "transparent", border: `1.5px solid ${COLORS.border}`, color: COLORS.textMuted, padding: "14px 24px", borderRadius: 12, cursor: "pointer", fontSize: 15 }}>Abbrechen</button>
                    <button onClick={handleLogout} style={{ background: "transparent", border: `1.5px solid ${COLORS.border}`, color: COLORS.textMuted, padding: "14px 20px", borderRadius: 12, cursor: "pointer", fontSize: 15, marginLeft: "auto" }}>Ausloggen</button>
                  </div>
                </div>
              </div>
            )}
        ...
        {activeNav === "jobs" && (
          <div>
            <h1 style={{ fontSize: 32, fontWeight: 700, marginBottom: 8, letterSpacing: "-0.5px" }}>Jobs finden</h1>
            <p style={{ fontSize: 16, color: COLORS.textMuted, marginBottom: 40 }}>Direkte Links zu inklusiven Stellenbörsen — kein Schwerbehindertenausweis nötig.</p>

            <p style={{ fontSize: 12, fontWeight: 700, textTransform: "uppercase", letterSpacing: "0.1em", color: COLORS.textMuted, marginBottom: 16 }}>Inklusive Jobbörsen</p>
            <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 16, marginBottom: 40 }} className="two-col">
              {[
                ...
                { name: "Rehadat-Jobs", desc: "Stellenportal des Bundesministeriums für Arbeit — speziell für Menschen mit Behinderungen.", url: "https://www.rehadat-jobs.de", color: COLORS.green },
              ].map(j => (
                <a key={j.name} href={j.url} target="_blank" rel="noreferrer"
                  ...
                  <p style={{ fontSize: 14, color: COLORS.textMuted, lineHeight: 1.6, marginBottom: 14 }}>{j.desc}</p>
                  <span style={{ fontSize: 14, color: j.color, fontWeight: 600 }}>Zur Jobbörse →</span>
                </a>
              ))}
            </div>

            <p style={{ fontSize: 12, fontWeight: 700, textTransform: "uppercase", letterSpacing: "0.1em", color: COLORS.textMuted, marginBottom: 16 }}>Allgemeine Jobbörsen</p>
            <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr 1fr", gap: 12, marginBottom: 32 }} className="two-col">
              {[
                ...
                <a key={j.name} href={j.url} target="_blank" rel="noreferrer"
                  ...
                  <p style={{ fontSize: 13, color: COLORS.textMuted, lineHeight: 1.5 }}>💡 {j.tipp}</p>
                </a>
              ))}
            </div>
            ...
            <div style={{ background: COLORS.bgCard, border: `1px solid ${COLORS.purpleBorder}`, borderRadius: 14, padding: "24px 28px" }}>
              ...
              <p style={{ fontSize: 15, color: COLORS.textMuted, lineHeight: 1.6, marginBottom: 16 }}>Schick deinen Diffusion-Profillink direkt in der Bewerbung mit — inklusiv, ehrlich, ohne Stigma.</p>
        ...
        {activeNav === "vorbilder" && (
          <div>
            <h1 style={{ fontSize: 32, fontWeight: 700, marginBottom: 8, letterSpacing: "-0.5px" }}>Vorbilder</h1>
            <p style={{ fontSize: 16, color: COLORS.textMuted, marginBottom: 28 }}>Menschen die ihr Profil öffentlich teilen — als Inspiration.</p>
            ...
            {vorbilder.length === 0 ? (
              <div style={{ textAlign: "center", padding: "80px 20px", color: COLORS.textMuted }}>
                ...
                <p style={{ fontSize: 15, marginBottom: 24, color: COLORS.textMuted }}>Sei die erste Person — aktiviere "Als Vorbild sichtbar sein" in deinem Profil!</p>
                    ...
                    <a key={v.id} href={`/profil/${v.id}`}
                      ...
                      <div style={{ display: "flex", gap: 14, alignItems: "center", marginBottom: 14 }}>
                        ...
                        <div>
                          ...
                          {v.headline && <p style={{ fontSize: 13, color: COLORS.textMuted, lineHeight: 1.4 }}>{v.headline}</p>}
                        </div>
                      </div>
                      <div style={{ display: "flex", flexWrap: "wrap", gap: 6 }}>
        ...
        {activeNav === "pinnwand" && (
          <div>
            <h1 style={{ fontSize: 32, fontWeight: 700, marginBottom: 8, letterSpacing: "-0.5px" }}>Pinnwand</h1>
            <p style={{ fontSize: 16, color: COLORS.textMuted, marginBottom: 32 }}>Teile Erfahrungen, Tipps und Sorgen — anonym oder mit Namen.</p>

            {/* Neuer Beitrag */}
            <div style={{ background: COLORS.bgCard, border: `1px solid ${COLORS.border}`, borderRadius: 14, padding: "24px 28px", marginBottom: 28 }}>
              <p style={{ fontSize: 16, fontWeight: 600, marginBottom: 16, color: COLORS.textSecondary }}>Neuer Beitrag</p>
              <div style={{ display: "grid", gridTemplateColumns: "1fr auto", gap: 12, marginBottom: 14 }} className="two-col">
                ...
                <label style={{ display: "flex", alignItems: "center", gap: 8, fontSize: 14, color: COLORS.textMuted, cursor: "pointer", whiteSpace: "nowrap" }}>
            ...
            {/* Beiträge */}
            {pinnwandLoading ? (
              <p style={{ color: COLORS.textMuted, textAlign: "center", padding: "40px 0", fontSize: 16 }}>Lädt…</p>
                    ...
                    <div key={b.id} style={{ background: COLORS.bgCard, border: `1px solid ${COLORS.border}`, borderRadius: 12, padding: "18px 22px" }}>
                      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 12 }}>
                        <div style={{ display: "flex", gap: 10, alignItems: "center", flexWrap: "wrap" }}>
                          ...
                          <span style={{ fontSize: 13, color: COLORS.textMuted }}>{b.anonym ? "Anonym" : b.anzeigename || "Nutzer:in"}</span>
                          <span style={{ fontSize: 12, color: COLORS.textMuted }}>{new Date(b.created_at).toLocaleDateString("de-DE")}</span>
                        </div>
                        {b.user_id === user?.id && (
                          <button onClick={() => deleteBeitrag(b.id)} style={{ background: "none", border: "none", color: COLORS.textMuted, cursor: "pointer", fontSize: 18, padding: "0 4px" }} title="Löschen">×</button>
                        )}
                      </div>
                      <p style={{ fontSize: 15, color: COLORS.textSecondary, lineHeight: 1.75 }}>{b.inhalt}</p>
                    </div>
                  );
                })}
              </div>
      ...
      <footer style={{ borderTop: `1px solid ${COLORS.border}`, padding: "20px 24px", textAlign: "center" }}>
        <p style={{ fontSize: 13, color: COLORS.textMuted }}>
          <a href="/impressum" style={{ color: COLORS.textMuted, textDecoration: "none" }}>Impressum & Datenschutz</a>
